    ...
    try:
        ...
        if isinstance(health, dict) and health:
            ...
            for symbol, item in sorted(health.items()):
                ...
                bar_close_stamp = _iso_datetime(identity["bar_close_utc"])
                next_eligible = item.get("next_eligible_at")
                next_eligible_stamp = _iso_datetime(str(next_eligible)) if next_eligible else None
                if stamp > now or bar_close_stamp > stamp or bar_close_stamp > now or (next_eligible_stamp and next_eligible_stamp <= stamp):
