...
function MarketRecord({ record }: { record: EvidenceRecord }) {
  ...
  return <article className="rounded-xl border border-slate-700/70 bg-white/[.02] p-5">
    ...
    {record.status !== "available" ? <div className="mt-4 border-t border-slate-700/70 pt-4 text-sm text-amber"><p>Severity: {record.status}</p><p className="mt-1">Definition: Market value is withheld unless its observed provider timestamp passes the configured freshness threshold.</p><p className="mt-1">Operator action: Refresh and validate the mounted market snapshot before relying on this instrument.</p></div> : null}
  </article>;
}
