...
export function OverviewIntegration({ fallback }: { fallback: React.ReactNode }) {
  ...
  // Keep the fallback state name for the integration contract; a failed API must never render a Mock fallback or any supplied demo values. The local snapshot is unavailable; no snapshot and mock values are combined.
  if (state.kind === "fallback") return <ErrorState title="Overview data unavailable" description="The read-only overview source could not be loaded. No snapshot and mock values are combined; operator action: restore the validated mounted sources." />;

  const { data } = state;
  const fresh = data.snapshot_freshness.status === "fresh";
  const points = fresh ? data.performance_series.items.map((item, index) => ({ index, sourceAsOf: item.as_of_utc, portfolio: Number(item.equity) })) : [];
  ...
  return <div className="space-y-6"><div className="space-y-2 text-[15px] text-slate-300"><p><StatusBadge label="Local snapshot" tone="neutral" /> <span className="ml-2">· {data.source_classification === "partial" ? "Partial" : "Local snapshot"}</span></p><p>Portfolio data as of {formatUtc(data.source_as_of_utc)}</p><p>Loaded {formatUtc(data.generated_at_utc)}</p>{data.snapshot_freshness.status === "stale" && ageDays !== null ? <p className="text-amber">Stale snapshot — portfolio data is {ageDays} days old</p> : null}</div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
    { label: "Portfolio Value", value: fresh ? displayMoney(data.portfolio_summary.portfolio_value) : "Stale / unavailable", helper: `Source: portfolio snapshot; UTC: ${formatUtc(data.source_as_of_utc)}; Definition: latest observed equity; Severity: ${fresh ? "observed" : "stale"}; Operator action: ${fresh ? "none" : "refresh and validate the mounted snapshot."}`, tone: fresh ? "neutral" as const : "warning" as const },
    { label: "Latest recorded change", value: fresh ? displayPercent(data.portfolio_summary.daily_change_percent) : "Stale / unavailable", helper: `Source: portfolio snapshot; UTC: ${formatUtc(data.portfolio_summary.latest_recorded_change_as_of_utc)}; Definition: observed recorded daily change; Severity: ${fresh ? "observed" : "stale"}; Operator action: ${fresh ? "none" : "refresh and validate the mounted snapshot."}`, tone: fresh ? "positive" as const : "warning" as const },
    { label: "Total Return", value: fresh ? displayPercent(data.portfolio_summary.total_return_percent) : "Stale / unavailable", helper: `Source: portfolio snapshot; UTC: ${formatUtc(data.source_as_of_utc)}; Definition: derived change from first to latest observed equity; Severity: ${fresh ? "derived" : "stale"}; Operator action: ${fresh ? "none" : "refresh and validate the mounted snapshot."}`, tone: fresh ? "positive" as const : "warning" as const },
    { label: "Cash", value: "Unavailable", helper: "Source: no explicit cash artifact; UTC: no timestamp; Definition: cash balance; Severity: unavailable; Operator action: mount a timestamp-matched cash source.", tone: "warning" as const },
