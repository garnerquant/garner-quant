...
st.caption("Current Live Strategy Summary")
...
st.caption(
    f"Journal events: {journal_events} | Open BUY/HOLD signals: "
    f"{open_signal_count} | Current signals count: {current_signals_count}"
)

with st.expander("Open research experiment tools", expanded=False):
...
st.divider()
with st.expander("Safety Rules", expanded=False):
    st.markdown(
        "\n".join(
            [
